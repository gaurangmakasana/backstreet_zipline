...
def _pricing_iter(csvdir, symbols, metadata, divs_splits, show_progress):
    with maybe_show_progress(symbols, show_progress,
        ...
        # print(files) # debug added
        for sid, symbol in enumerate(it):
            logger.debug('%s: sid %s' % (symbol, sid))
            # print(sid) # debug added
            # print(symbol) # debug added

            try:
                fname = [fname for fname in files
                        #  if '%s.csv' % symbol in fname][0]  # it looks like this is what is f3$%ing it up
                         if '%s.csv' % symbol == fname][0]  # fixed it here
                # print(fname) # debug added
            ...
            # Sync to the official trading calendar, I'll probably just handle this on the dataside
            ...
            if 'dividend' in dfr.columns:
                ...
                div['pay_date'] = NaT
                # add a pay_date - This fixed the problem
